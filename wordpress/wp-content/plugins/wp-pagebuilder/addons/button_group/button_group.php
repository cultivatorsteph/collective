				...
				foreach ( $button_item as $key => $value ) {
					...
					if( !empty($value['button_link']['link'])) {
						...
						if( isset($value['icon_position']) == 'left') {
							$button = (esc_attr( isset($value['icon_list'] ))) ? '<i class="' . esc_attr( $value['icon_list']) . '"></i> ' . esc_attr($value['button_text']) : esc_attr($value['button_text']);
						} elseif ( isset($value['icon_position']) == 'right') {
							$button = (esc_attr( isset($value['icon_list']))) ? esc_attr($value['button_text']) . ' <i class="' . esc_attr( $value['icon_list']) . '"></i>' : esc_attr($value['button_text']);
						} else {
							$button = (esc_attr( isset($value['icon_list']))) ? esc_attr($value['button_text']) . ' ' : esc_attr($value['button_text']);
						}
		...
		<div class="wppb-button-group-addon">
			<div class="wppb-button-addon-content">
				...
				<a {{ value.button_link.link ? "href="+value.button_link.link : "" }} {{ value.button_link.window ? "target=_blank" : "" }} {{ value.button_link.nofolow ? "rel=nofolow" : "" }}  class="wppb-btn-addons {{classList}}">
					<# if(value.icon_position == "left") { #><i class="{{ value.icon_list }}"></i> <# } #>{{ value.button_text }}<# if(value.icon_position == "right") { #> <i class="fa {{ value.icon_list }}"></i><# } #>
				</a>
				<# } #>
				</div>
