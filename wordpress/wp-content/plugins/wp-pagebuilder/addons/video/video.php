...
class WPPB_Addon_Video{
	...
	public function get_settings() {
		$settings = array(
			...
			'video_info' => array(
				'type' => 'switch',
				'title' => __('Video info','wp-pagebuilder'),
				'std' => '1'
			),
			//style
			'video_width' => array(
			...
			'video_radius' => array(
				...
				'tab' => 'style',
				'unit' => array( 'px','%','em' ),
				'selector' => '{{SELECTOR}} .wppb-video-block.wppb-embed-responsive iframe { border-radius: {{data.video_radius}}; }'
			),
		);
	...
	public function render($data = null){
		...
		$video_info 	= isset($settings['video_info']) ? $settings['video_info'] : '';
		$output = '';
		if($video_url) {
			...
			switch($video['host']) {
				case 'youtu.be':
				$id = trim($video['path'],'/');
				$src = '//www.youtube.com/embed/' . $id.'?&showinfo='.esc_attr($video_info);
				break;
			...
			$output .= '<iframe class="wppb-embed-responsive-item" src="' . $src . '" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
			$output .= '</div>';
			$output .= '</div>';
			$output .= '</div>';
			return $output;
	...
	public function getTemplate(){
				...
				switch( videoObj.host ){
					case "youtu.be":
						var videoId = videoObj.path.trim();
								embedSrc = "//www.youtube.com/embed"+ videoId + "?&showinfo="+data.video_info
						break;

					case "www.youtube.com":
					case "youtube.com":
						var queryStr = videoObj.query.split("=");
								embedSrc = "//www.youtube.com/embed/"+ queryStr[1] + "?&showinfo="+data.video_info
						break;
		...
		<div class="wppb-video-addon">
			<div class="wppb-video-content">
				...
				<# if(embedSrc){ #>
					<iframe class="wppb-embed-responsive-item" src=\'{{ embedSrc }}\' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
				<# } #>
				</div>
			</div>
		</div>
