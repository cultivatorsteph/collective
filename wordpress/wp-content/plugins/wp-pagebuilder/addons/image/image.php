...
class WPPB_Addon_Image{
	...
	public function get_settings() {
		...
		$settings = array(
			...
			'overlay_icon' => array(
				...
				'selector' => '{{SELECTOR}} .wppb-addon-image-overlay-icon { background: {{data.overlay_icon}}; }'
			),
	...
	public function render($data = null){
		...
		if($lightbox) {
			$output  .= '<div class="wppb-addon-image-overlay">';
			$output .= '<a class="wppb-image-popup wppb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href="'. esc_url($img_url) .'">+</a>';
			$output .='</div>';
		}
		if( !$lightbox && get_wppb_array_value_by_key($image_link, 'link') ) {
			$output  .= '<a '.esc_attr($nofolow).' href="'.esc_url(get_wppb_array_value_by_key
				($image_link, 'link')).'" '.esc_attr($target).'>';
		}
				...
				<div class="wppb-image-addon-content wppb-{{ data.image_display }}">
					<# if( data.lightbox == 1 ) { #>
						<div class="wppb-addon-image-overlay">
							<a class="wppb-image-popup wppb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href="{{data.upload_image.url }}">+</a>
						</div>
					<# } #>
