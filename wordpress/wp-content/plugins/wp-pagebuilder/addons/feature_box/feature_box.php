	...
	public function render($data = null){
		...
		if($show_image == 1) {	
			if ( ! empty($image_upload['url']) ) {
				...
				if( isset( $image_animation['itemOpen'] ) ){
			...
			if( !empty( $icon_list ) ){
				if( isset( $icon_animation['itemOpen'] ) ) {
		...
		if( !empty($button_link['link']) ){
			$btntarget = isset($button_link['window']) ? "target=_blank" : "target=_self";
			$btnnofolow = isset($button_link['nofolow']) ? "rel=nofolow" : "";
