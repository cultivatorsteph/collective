		...
		public function get_settings() {
			...
			$settings = array(
				...
				'carosuel_list' => array(
					...
					'attr' => array(
						...
						'carosuel_bg_overlay' => array(
							'type' => 'color2',
							'title' => __('Background overlay','wp-pagebuilder'),
							'section' => 'Background',
							'clip' => false,
							'std' => array(
								'clip' => false,
				...
				'nav_height' => array(
					...
					'tab' => 'style',
					'section' => 'Nav',
					'selector' => '{{SELECTOR}} .wppb-carousel .slick-arrow { height: {{data.nav_height}}; },
					{{SELECTOR}} .wppb-carousel .slick-arrow { line-height: {{data.nav_height}}; }'
				),
				'nav_radius' => array(
					'type' => 'dimension',
					'title' => __('Border Radius','wp-pagebuilder'),
                ...
                <div class="wppb-carousel stick-dots" data-addon-id="<?php echo $data['id'];?>" data-settings="<?php echo esc_attr(json_encode($attrSettings));  ?>" >
					...
					if ( get_wppb_array_value_by_key( $value, 'media_type' ) == "media_video" ) {
						if ( $video_url ) {
							...
							$ImageMedia .= '<iframe height="280" width:="100%" class="wppb-embed-responsive-item" src="' . $videosrc . '" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
							$ImageMedia .= '</div>';
						} else {
					...
					<div class="wppb-carousel stick-dots" data-addon-id="{{data.id}}">
						<#  _.forEach(data.carosuel_list, function(value, key) { #>
							...
							if ( value.media_type == "media_video" ) {
								if ( videoUrl ) {
									ImageMedia  = "<div class=\'wppb-video-slide\'><iframe height=\'280\' width:=\'100%\' class=\'wppb-embed-responsive-item\' src=\'"+videosrc+"\' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>";
								}
							} else {
