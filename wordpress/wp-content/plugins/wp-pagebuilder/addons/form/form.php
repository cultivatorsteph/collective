	...
	public function get_settings() {
		...
		$settings = array(
			...
			'label_color' => array(
				...
				'std'     => '#353535',
				'depends' => array(array('label_show', '=', '1')),
				'selector' => '{{SELECTOR}} .wppb-form-field-label, {{SELECTOR}} .wppb-global-form-content label, {{SELECTOR}} label { color: {{data.label_color}}; min-height: 24px;
				min-width: 20px; }'
			),
            ...
            <form method="post" enctype="multipart/form-data" class="wppb-form-addon">
                ...
                <div class="wppb-form-field-item-wrap">
					...
					foreach ($input_items as  $fieldIndex => $field){
						...
						if ( isset($field['require']) && (int) $field['require'] === 1) {
							...
							$fieldAttrA['data-label'] = isset($field['label']);
						}
				...
				<?php if ($enableSimpleRecaptcha){ ?>
                    <div class="wppb-custom-recaptcha">
                        <span class="wppb-form-recaptcha-input"><input type="text" name="wppb_default_form[wppb_form_recaptcha_answer]" data-required="true" placeholder="10+5 = ?" data-label="<?php _e("WPPB Simple reCaptcha", 'wp-pagebuilder'); ?>"
                            /></span>
                    </div>
				<?php } ?>
                ...
                <div class="wppb_form_addon_submit_btn_wrap wppb-form-btn-wrap">
                    <button class="wppb-btn-forms" type="submit" class="wppb_form_addon_submit"> <?php echo $button; ?> </button>
                </div>
