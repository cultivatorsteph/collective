jQuery(document).ready(function($){ 'use strict';
    ...
    /* 
    function setWidth(){
        let width = $( window.parent.document.getElementById( 'wppb-builder-view' ) ).outerWidth(true)
        $( ".wppb-row-parent" ).each( function( index ) {
            $(this).css( {'width': width, 'left':  '-' + ( $('.wppb-builder-blank-page-tools').offset().left ) + 'px'} );
        });
    }
    $(window).on( 'load', function(e){ setWidth(); });
    $(window).on( 'resize', function(e){ setWidth(); });
    jQuery( document ).on( "setWidthTrigger", function() { setWidth(); });
    */
    ...
    $(document).on('rendered_addon', function(e, addon){
        if (typeof addon.type !== 'undefined' && ( addon.type === 'addon' || addon.type === 'inner_addon' ) && ( addon.name === 'wppb_testimonial_carousel' || addon.name === 'wppb_person_carousel' )){
            ...
            if( Container.length > 0 ){
                slickSliderCallback( Container.data('autoplay') , Container.data('showdots'), Container.data('shownav'), Container.data('col'), Container, 'backend' );
            }
        }
    });
    ...
    if( testimonial.length > 0 ){
        slickSliderCallback( testimonial.data('autoplay') ,testimonial.data('showdots'), testimonial.data('shownav'), testimonial.data('col'), testimonial, 'frontend' );
    }
    if( person.length > 0 ){
        slickSliderCallback( person.data('autoplay') ,person.data('showdots'), person.data('shownav'), person.data('col'), person, 'frontend' );
    }
    function slickSliderCallback( auto, dot, nav, col, Container, type ){
        let argument = {
            autoplay: auto,
            dots: dot,
            arrows: nav,
        ...
        if( nav == true ){
            argument.nextArrow = '<span class="wppb-carousel-next"><i class="fa fa-angle-left"></i></span>';
            argument.prevArrow = '<span class="wppb-carousel-prev"><i class="fa fa-angle-right"></i></span>';
        }
        if( type == 'backend' ){
